...
makeFastaSilvaNR <- function(fin, ftax, fout_fasta, fout_taxonomy,
  ...
  makeFastaSilvaNR <- function(fin, ftax, fout_fasta, fout_taxonomy,
    ...
    # Generate final taxonomy strings with exactly 7 ranks for each reference ID
    taxa.string.final <- apply(taxa.mat.final, 1, function(x) {
      # Ensure we have exactly 7 taxonomy levels
      # Replace NA with empty string to maintain position
      levels <- ifelse(is.na(x), "", x)

      # Make sure we have exactly 7 levels (pad with empty if needed)
      if (length(levels) < 7) {
        levels <- c(levels, rep("", 7 - length(levels)))
      } else if (length(levels) > 7) {
        levels <- levels[1:7]
      }
    ...
    # Set headers for FASTA output - use only the clean IDs
    names(xset.out) <- names(taxa.string.final)

    # Write FASTA output
    writeFasta(ShortRead(xset.out), fout_fasta, width = 20000L, compress = compress)

    # Write taxonomy file - properly formatted with clean reference IDs
    taxonomy_df <- data.frame(
      ID = names(taxa.string.final),
      Taxonomy = taxa.string.final,
      stringsAsFactors = FALSE
    )
    write.table(taxonomy_df, fout_taxonomy, sep = "\t", quote = FALSE,
